...
export const isURL = (url) => {
  const reg = /^https?:\/\/((([a-zA-Z\d]([a-zA-Z\d-]*[a-zA-Z\d])*)\.)+[a-zA-Z]{2,}|((\d{1,3}\.){3}\d{1,3}))(:\d+)?(\/[^\s]*)?$/;
  return reg.test(url);
};
 ...
 * 作用：判断本地存储的值是否为空
 */
export const isStorageFieldEmpty = (val) => isValueEmpty(val) || val === "undefined" || val === "null";

/**
 * 作用：判断一个字段的值是否为空
 */
export const isValEmpty = (val) => val === null || val === undefined || (typeof val === "string" && val.trim() === "");
...
export const getAgeFromIDCard = (idCardNumber) => {
  ...
  return moment(getServeTime()).diff(handleMoment(getBirthDateFromIDNumber(idCardNumber), DATE_FORMAT), "years");
};
