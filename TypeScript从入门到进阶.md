...
```ts
let xiaoming:[string,number] = ['xiaoming',5];
xiaoming[0].length;
...
```ts
const animal:[string,number,boolean] = ['xiaoming',10,true];
```
...
let result1 = error('hello');// 由类型推论得到返回值为 never
function fail() {
...
- 原始数据类型（布尔值，数值，字符串，null，undefined，Symbol，BigInt）

```ts
let name = 'xiaoming';console.log(name.toUpperCase());
console.log((new String('xiaoming')).toUpperCase());
```
...
name = 3;console.log(name.toFixed(2));
name = 'xiaoming';console.log(name.length);
export {};
```
...
let com = Component;//Component类名本身表示的是实例的类型
//ts 一个类型 一个叫值 
//冒号后面的是类型
//放在=后面的是值
...
```js
"use strict";var User = /** @class */ (function () {
...
user.name = 'Tom';console.log(user.name);
```
...
class Student extends Person{
    no: string;
    constructor(name:string,age:number,no:string) {
        super(name,age);
        this.no=no;
    }
    getNo():string {
...
let child = new Child('xmpx',10,1000);
...
let dog=new Dog();let cat=new Cat();
dog.speak();
cat.speak();
```
...
let cat = new Cat();console.log(cat.speak('hello'));//--------------------------------------------
...
let cat = new Cat();console.log(cat.speak('hello'));let dog = new Dog();console.log(dog.speak('hello'));
```

## 6接口

- 接口一方面可以在面向对象编程中表示为行为的抽象，另外可以用来描述对象的形状(shape)
...
let arr:UserInterface = ['xmpx1','xmpx2'];
console.log(arr);

interface UserInterface2 {
  [index:string]:string
}
let obj:UserInterface2 = {name:'xiaoming'};
```
...
let a = createAnimal(Animal,'xiaoming');
console.log(a.name);
```
...
getName(p);//只有在传参的时候两个变量之间才会进行兼容性的比较，赋值的时候并不会比较,会直接报错
let a: Animal = {
    name: 'xiaoming',
    age: 10,
    gender: 0
...
```ts
//基本数据类型也有兼容性判断
let num : string|number;let str:string='xiaoming';
num = str;
...
```ts
type sumFunc = (a:number,b:number)=>number;let sum:sumFunc;function f1(a:number,b:number):number{
  return a+b;
}
sum = f1;
//可以省略一个参数
...
```ts
type GetPerson = ()=>{name:string,age:number};let getPerson:GetPerson;//返回值一样可以
function g1(){
    return {name:'xiaoming',age:10};
}
getPerson = g1;//返回值多一个属性也可以
function g2(){
    return {name:'xiaoming',age:10,gender:'male'};
}
getPerson = g2;//返回值少一个属性可不行
function g3(){
    return {name:'xiaoming'};
}
getPerson = g3;//因为有可能要调用返回值上的方法
...
}let animal: Animal;let blackDog: BlackDog;let whiteDog: WhiteDog;
type Callback = (dog: Dog)=>Dog;function exec(callback:Callback):void{
    callback(whiteDog);
}//不行  callback(redDog);
type ChildToChild = (blackDog: BlackDog) => BlackDog;const childToChild: ChildToChild = (blackDog: BlackDog): BlackDog => blackDog
exec(childToChild);
//也不行,理由同上
type ChildToParent = (blackDog: BlackDog) => Animal;const childToParent: ChildToParent = (blackDog: BlackDog): Animal => animal
exec(childToParent);
//不行 因为有可能调用返回的Dog的方法
type ParentToParent = (animal: Animal) => Animal;const parentToParent: ParentToParent = (animal: Animal): Animal => animal
exec(parentToParent);
//可以,所有的狗都是动物,返回的不管什么狗都是狗
type ParentToChild = (animal: Animal) => BlackDog;const parentToChild: ParentToChild = (animal: Animal): BlackDog => blackDog
exec(parentToChild);//(Animal → Greyhound) ≼ (Dog → Dog)
//返回值类型很容易理解：黑狗是狗的子类。但参数类型则是相反的：动物是狗的父类！

TypeScript
// string | number|boolean 是 string | number的父类型
// string是string|number的子类型
type Callback2 = (a: string | number) => string | number;function exec2(callback: Callback2):void{
    callback('');
}
type ParentToChild2 = (a: string | number | boolean) => string;const parentToChild2: ParentToChild2 = (a: string | number | boolean): string => ''
exec2(parentToChild2);

type Callback3 = (a: string | number) => string | number;function exec3(callback: Callback2): void {
    callback('');
}
type ParentToParent3 = (a: string) => string;const parentToParent3: ParentToParent3 = (a: string): string => ''
...
interface Empty<T>{}let x!:Empty<string>;let y!:Empty<number>;
x = y;
//2.接口内容不为空的时候不可以
interface NotEmpty<T>{
  data:T
}let x1!:NotEmpty<string>;let y1!:NotEmpty<number>;
x1 = y1;
...
interface NotEmptyNumber{
    data:number
}let xx2!:NotEmptyString;let yy2!:NotEmptyNumber;
xx2 = yy2;
```
...
type Button = WarningButton|DangerButton;function getButton(button:Button){
...
```ts
type UnionType1 = unknown | 
null
;       
// unknown

type UnionType2 = unknown | 
undefined
;  
// unknown
...
```ts
type Sum = (a: number, b: number) => number;let sum: Sum = (a, b) => {
    a='xiaoming';
    return a + b;
};
```
...
type BirdPerson = Bird & Person;let p: BirdPerson = { name: 'xiaoming', fly() { }, talk() { } };
p.fly;
p.name
p.talk;
```
...
const x = mixin({ name: "xiaoming" }, { age: 11 });
...
function getValueByKey(p:Person,key:PersonKey){
  return p[key];
}let val = getValueByKey({name:'xiaoming',age:10,gender:'male'},'name');console.log(val);
```
...
type Condition<T> = T extends Fish ? Water : Sky;let condition: Condition<Fish> = { name: '水' };
```
...
// Water|Sky
let condition1: Condition<Fish | Bird> = { water: '水' };let condition2: Condition<Fish | Bird> = { sky: '天空' };
```
...
type  E = Extract<string|number,string>;let e:E = '1';
```
...
```ts
export {}
type ReturnType<T extends (...args: any[]) => any> = T extends (...args: any[]) => infer R ? R : any;function getUserInfo() {
    return { name: "xiaoming", age: 10 };
}
...
type constructorParameters = ConstructorParameters<typeof Person>;let params: constructorParameters = ['xiaoming']//实例类型
type Instance = InstanceType<typeof Person>;let instance: Instance = { name: 'xiaoming', getName() { } };
```
...
- 「Distributive conditional types」是由「naked type parameter」构成的条件类型。而「naked type parameter」表示没有被 Wrapped 的类型（如：Array
...
let person: Person = { name: 'xiaoming', age: 10, married: true };
...
type Proxify<T> = {
    ...
    name: 'xiaoming',
    age: 10
}let proxyProps = proxify(props);console.log(proxyProps);
...
let originProps = unProxify(proxyProps);console.log(originProps);
```
...
let a1:Animal={name:'xiaoming',age:10};
console.log(a1.name);
console.log(a1.age);
//注意不要加export {} ,这是全局的
```
